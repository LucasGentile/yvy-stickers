...
- _Missing mock_ — a new import wasn't mocked in the test file. Add the mock.
- _Stale test_ — the test asserts behaviour that was intentionally removed. Update the test to match current intent (or delete if the feature is gone).
- _Real regression_ — the code broke something. Fix the code.
