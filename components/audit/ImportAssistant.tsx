          ...
          {sortedAlbum.length > 0 && (
            <div>
              <div className="flex items-baseline justify-between mb-3">
                ...
                <p
                  className={`text-[11px] font-medium ${allAlbumDone ? 'text-green-600' : 'text-yvy-muted'}`}
                >
                  {allAlbumDone
                    ? 'Tudo colado ✓'
                    : `${checkedAlbum.size}/${sortedAlbum.length} coladas`}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                {sortedAlbum.map((id) => (
                  <StickerChip
                    ...
                    variant="green"
                    size="md"
                    checked={checkedAlbum.has(id)}
                    onToggle={() => toggle(checkedAlbum, setCheckedAlbum, id)}
                  />
                ))}
              </div>
            </div>
            ...
            <div>
              <div className="flex items-baseline justify-between mb-3">
                <p className="text-xs font-bold uppercase tracking-wide text-amber-500">
                  Para separar como repetidas →
                </p>
                <p
                  className={`text-[11px] font-medium ${allDupesDone ? 'text-green-600' : 'text-yvy-muted'}`}
                >
                  {allDupesDone
                    ? 'Tudo separado ✓'
                    : `${checkedDupes.size}/${sortedDupes.length} separadas`}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                {sortedDupes.map((id) => (
                  <StickerChip
                    ...
                    variant="amber"
                    size="md"
                    checked={checkedDupes.has(id)}
                    onToggle={() => toggle(checkedDupes, setCheckedDupes, id)}
                  />
                ))}
              </div>
            </div>
