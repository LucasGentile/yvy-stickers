...
- [x] 3.3 Implement client-side validation: all fields required, apartment = 4 chars, tower = 1–2 chars
...
- [x] 6.1 Build `StickerGrid` component: CSS grid of 980 checkboxes labeled 1–980
...
- [x] 6.4 Add a "Select all" / "Clear all" helper for usability
- [x] 6.5 Verify grid renders and scrolls smoothly on a mobile viewport

## 7. Trade Matching

- [x] 7.1 Create `app/matches/page.tsx` as Screen 3 (match results)
- [x] 7.2 Implement `lib/matching.ts`: query all other users' stickers from Supabase and compute `matchScore` and `reciprocalScore` in-process
- [x] 7.3 Sort results by `matchScore` DESC, then `reciprocalScore` DESC
- [x] 7.4 Build `MatchCard` component: displays display key, matchScore ("X figurinhas que eu preciso"), reciprocalScore ("X figurinhas que tenho para dar"), and WhatsApp button
- [x] 7.5 Handle empty state: show pt-BR message when no matches found
- [x] 7.6 Add "Atualizar minhas figurinhas" button linking back to `/stickers`

## 8. WhatsApp Integration

- [x] 8.1 Build `WhatsAppButton` component: renders `<a href="https://wa.me/<phone>">` for per-user contact
- [x] 8.2 Build persistent header with app name and condominium group button (`NEXT_PUBLIC_WHATSAPP_GROUP_URL`)
- [x] 8.3 Add header to root layout so it appears on all screens

## 9. Testing

- [x] 9.1 Install and configure Vitest with `@vitejs/plugin-react` and `@testing-library/react` for unit/integration tests
- [x] 9.2 Install and configure Playwright for E2E tests (`npx playwright install`)
- [x] 9.3 Unit test `lib/parser.ts`: valid input, semicolon-separated, spaces stripped, deduplication, out-of-range rejection, empty input
- [x] 9.4 Unit test `lib/normalize.ts`: lowercase, trim, display key format (`name-apartment-tower`)
- [x] 9.5 Unit test `lib/matching.ts`: correct matchScore, correct reciprocalScore, sort order (primary + tiebreaker), empty result when no overlap
- [x] 9.6 Integration test `actions/registerUser.ts`: new user created, duplicate phone returns existing user, missing fields rejected (mock Supabase client)
- [x] 9.7 Integration test `actions/saveStickers.ts`: stickers inserted, re-submission replaces previous data idempotently (mock Supabase client)
- [x] 9.8 E2E test — Registration flow: fill form with valid data → redirected to `/stickers`
- [x] 9.9 E2E test — Sticker input via file upload: upload valid `.txt` → success confirmation shown
- [x] 9.10 E2E test — Full happy path: register → select mode → upload file → view matches → WhatsApp button has correct `href`
- [x] 9.11 Configure Vitest coverage reporter; verify overall coverage is ≥ 80%

## 10. Polish & QA

- [x] 10.1 Verify all user-facing strings are in pt-BR
